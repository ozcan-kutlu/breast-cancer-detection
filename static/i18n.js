...
(function (global) {
  ...
  const STRINGS = {
    tr: {
      ...
      "model.treeLoadError":
        "Ağaç görseli yüklenemedi. python train.py ile PNG üretin ve sunucuyu yeniden başlatın.",
    ...
    en: {
      ...
      "model.treeLoadError":
        "Tree image failed to load. Run python train.py to build the PNG and restart the server.",
  ...
  function boot() {
    ...
    refreshLangButtons();
    document.title = t(
      document.body?.dataset?.appPage === "model" ? "title.model" : "title.predict"
    );
    initLangSwitch();
  }
  ...
  function initLangSwitch() {
    document.querySelectorAll("[data-lang-set]").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        applyPageI18n();
        document.title = t(
          document.body?.dataset?.appPage === "model"
            ? "title.model"
            : "title.predict"
        );
